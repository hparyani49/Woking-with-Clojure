        ...
        [sa/Form
         ...
         [sa/Button
          {:type "submit"
           ...
           :disabled (not (and @input-value (date/date-read @input-value (loc i18n :date-format))))} "Find"]]))))
           ...
           [sa/TableRow
            [sa/TableHeaderCell {:colSpan 1 :collapsing true}
             [sa/Button
              {:type "button"
               :animated true
               :onClick (fn [_] (swap! month-x (fnil dec 0)) nil)}
              [sa/ButtonContent {:visible true} "Prev"]
              [sa/ButtonContent {:hidden true} [sa/Icon {:name "left arrow"}]]]]
            [sa/TableHeaderCell {:colSpan 10 :collapsing true :textAlign "center"} month-name]
            [sa/TableHeaderCell {:colSpan 1 :collapsing true}
             [sa/Button
              {:type "button"
               :animated true
               :onClick (fn [_] (swap! month-x (fnil inc 0)) nil)}
              [sa/ButtonContent {:visible true} "Next"]
              [sa/ButtonContent {:hidden true} [sa/Icon {:name "right arrow"}]]]]]
           [sa/TableRow
            [sa/TableHeaderCell {:colSpan 1}]
            (if week-numbers? [sa/TableHeaderCell (loc i18n :week-short)])
            (for [day (first month)
                ...
                [:td
                 ...
                 [sa/Dropdown
                  {:text day-num
                   :icon ""}
                  [sa/DropdownMenu
                   [sa/DropdownItem {:text "Complete this"}]
                   [sa/DropdownItem {:text "die"}]]]])])]]]))))


#_[sa/TableHeaderCell {:colSpan (if week-numbers? 8 7)}
             [sa/Header
              [sa/Button
               {:type "button"
                :animated true
                :onClick (fn [_] (swap! month-x (fnil dec 0)) nil)}
               [sa/ButtonContent {:visible true} "Prev"]
               [sa/ButtonContent {:hidden true} [sa/Icon {:name "up arrow"}]]]
              [:span.calendar-header__month-name month-name]
              #_[:div.calendar-header__content
               (if date-input?
                 [date-input
                  {:value value
                   :text text
               ...
               {:type "button"
                ...
                :onClick (fn [_] (swap! month-x (fnil inc 0)) nil)}
               [sa/ButtonContent {:visible true} "Next"]
               [sa/ButtonContent {:hidden true} [sa/Icon {:name "down arrow"}]]]]]
#_[:p
 {:class (str "calendar__date-button "
  ...
  :type "text"
  :on-click (fn [e] [sa/Dropdown {:fluid true :selection true}])
  :disabled disabled?}
 day-num]
